...
say "nginx sites directory"
# The engine writes a vhost per marketing_domain at runtime, as `sme`. Give it its own directory
# rather than write access to the shared sites-enabled: nginx.conf already globs conf.d, so one
# additive drop-in wires it up without editing a single shared file.
SITES_DIR="${SME_NGINX_SITES_ROOT:-/etc/nginx/sme-sites}"
install -d -o "$SME_USER" -g "$SME_USER" -m 755 "$SITES_DIR"
printf '# Generated by provision.sh (S0.5, #80) — engine-managed vhosts live here.\ninclude %s/*.conf;\n' \
    "$SITES_DIR" > /etc/nginx/conf.d/sme-sites.conf
chmod 644 /etc/nginx/conf.d/sme-sites.conf
echo "engine vhost dir $SITES_DIR (owned by $SME_USER), included via conf.d/sme-sites.conf"
