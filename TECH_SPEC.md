...
- **Check port conflicts before binding** — and treat this as a live check, not a documented fact. v1 planned FastAPI `:8010` / dashboard `:3010`; by the time S0.5 (#80) deployed, both had been taken by *other projects on the same box*, so the engine binds **`:8020` (API) and `:3020` (dashboard)**, with `:5555` reserved for Flower. SQLite is a file (no port). `infra/deploy/check-ports.sh` runs on every deploy and `deploy.sh` aborts non-zero on a conflict. See `infra/deploy/PORTS.md` for the full survey.
- **The box is shared with four unrelated projects.** Never edit an existing nginx vhost; never reload nginx without `nginx -t` first (`infra/deploy/nginx-reload.sh` is the only sanctioned path). "System" paths are not necessarily system installs — `/usr/local/bin/uv` there is a symlink into another user's home — so the deploy builds as the service user and provisioning copies toolchains to root-owned, world-readable locations.
- **Process supervision:** systemd units `sme-api` (uvicorn + the in-process worker/scheduler) and `sme-dashboard`, both `Restart=always` and enabled at boot. There is no separate Celery worker unit: every `media.*` task routes to the `media` queue consumed by the ephemeral GPU pod, and the `default` queue has no producers.
...
- **TLS** is issued with `certbot certonly --webroot` (not the nginx plugin, whose in-place vhost edits `deploy_site` would erase on the next `setup_site` run) and attached via a per-domain *wildcard* include the generated vhost carries. The ACME challenge location must be a `^~` prefix match so it outranks the `location ~ /\.` deny added in S4.5.1/#78 — otherwise every issuance times out silently. Renewal rides the box's existing `certbot.timer` with a deploy hook that reloads nginx. `infra/deploy/enable-tls.sh` automates all of it.
- **CORS** configured and asserted before first remote deploy (`infra/deploy/verify-deploy.sh`, which also asserts the private surface is loopback-bound). Deploy is idempotent and pull-based (`deploy.sh`); feature branch → PR → main; pre-commit hooks required. Runbook: `infra/deploy/RUNBOOK.md`.
